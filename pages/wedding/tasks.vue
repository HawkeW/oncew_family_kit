<template>
  <div class="space-y-6">
    ...
    <div class="flex gap-4 border-b pb-4 overflow-x-auto">
      ...
      <NuxtLink to="/wedding/tasks" class="px-4 py-2 rounded-lg hover:bg-muted bg-primary text-primary-foreground font-medium whitespace-nowrap">
        任务清单
      </NuxtLink>
    ...
    <!-- Task List -->
    <div class="space-y-4">
        <div v-if="filteredList.length === 0" class="text-center py-10 text-muted-foreground bg-card rounded-lg">
            暂无任务，快去添加吧！
        </div>
        ...
        <div v-else class="grid gap-4">
            <div 
                ...
                :key="task.id" 
                class="bg-card p-4 rounded-lg flex flex-col md:flex-row md:items-start gap-4 transition-all"
                ...
                <div class="flex items-start gap-3 w-full">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span v-if="task.due_date" class="text-xs px-2 py-0.5 rounded-full whitespace-nowrap" :class="isOverdue(task) ? 'bg-destructive/20 text-destructive' : 'bg-primary/20 text-primary'">
